    ...
    persist(
      (set, get): VerxioState => ({
        ...
        loadPlayerLoyalty: async (playerPublicKey: PublicKey) => {
          ...
          try {
            const loyalty =
              await verxioService.getPlayerLoyalty(playerPublicKey);
            ...
            if (loyalty?.guildId) {
              ...
              playerGuild =
                guilds.find((g) => g.id === loyalty.guildId) || null;
        ...
        awardLoyaltyPoints: async (
          ...
          try {
            const success = await verxioService.awardPoints(
              playerPublicKey,
              points,
              reason,
            );
        ...
        joinGuild: async (playerPublicKey: PublicKey, guildId: string) => {
          ...
          if (!verxioService)
            return { success: false, error: "Verxio service not available" };

          try {
            const result = await verxioService.joinGuild(
              playerPublicKey,
              guildId,
            );
            ...
            if (result.success) {
              ...
              if (playerLoyalty && guild) {
                ...
                const guildMembers =
                  await verxioService.getGuildMembers(guildId);
                const memberRank =
                  guildMembers.length === 1 ? "leader" : "member";
        ...
        updateReputation: async (
          ...
          try {
            const success = await verxioService.updateReputation(
              playerPublicKey,
              change,
              reason,
            );
        ...
        getPointsToNextTier: (currentPoints: number) => {
          ...
          return (
            verxioService?.getPointsToNextTier(currentPoints) || {
              needed: 0,
              nextTier: null,
            }
          );
