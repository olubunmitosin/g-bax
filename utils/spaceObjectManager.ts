...
export class SpaceObjectManager {
  ...
  public addSpaceObject(spaceObject: SpaceObject): ObjectMesh {
    ...
    const mesh = new THREE.Mesh(geometry, material) as ObjectMesh;
    
    // Set position, rotation, and scale
    mesh.position.set(...spaceObject.position);
    mesh.rotation.set(...spaceObject.rotation);
    mesh.scale.set(...spaceObject.scale);
    ...
    this.scene.add(mesh);
    this.objectMeshes.set(spaceObject.id, mesh);
  ...
  public getAllSpaceObjects(): SpaceObject[] {
    return Array.from(this.objectMeshes.values()).map(mesh => mesh.userData.spaceObject);
  }
    ...
    this.objectMeshes.forEach((mesh) => {
      ...
      switch (obj.type) {
        ...
        case 'resource_node':
          ...
          mesh.position.y = obj.position[1] + Math.sin(Date.now() * 0.001 + obj.position[0]) * 0.1;
          break;
        case 'station':
  ...
  public getSelectedObject(): SpaceObject | null {
    return this.selectedObject ? this.selectedObject.userData.spaceObject : null;
  }
